...
### table 생성 DDL인 `schema.sql`로 table이 만들어지는데, Entity의 `@Column` 애너테이션이 필요한가?
...
### delete/update 쿼리를 실행할 때 `@Modifying`이 필요한 이유
...
- [x] JPA 주요 애너테이션을 활용해 ERD, 연관관계 매핑 정보를 도메인 모델에 반영해보세요.
  - `@Entity`, `@Table`
  - `@Column`, `@Enumerated`

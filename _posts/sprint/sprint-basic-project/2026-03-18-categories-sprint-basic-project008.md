...
실행 로그와 바인딩된 파라미터를 확인했을 때, 분명 파라미터가 제대로 들어가는데 조회가 실패하는 원인을 파악하기 어려웠다. 이후 AI 도구의 도움을 받아 원인을 분석했고, PostgreSQL이 `LocalDate` 파라미터와 `IS NULL` 조건이 포함된 JPQL을 실행할 때 매개변수 타입을 추론하지 못한다는 점을 알게 되었다.

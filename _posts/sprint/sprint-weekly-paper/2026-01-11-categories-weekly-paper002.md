...
- **인터페이스 분리의 원칙(ISP) :** 하나의 범용 인터페이스보다 다수의 구체적인 인터페이스가 나으며, 클라이언트는 사용하지 않은 메서드에 의존하면 안된다.
...
여기서 새로운 결제 수단이 생길 때마다 `if-else`문이 늘어나면서 `pay()` 메서드가 수정되기 때문에 개방-폐쇄 원칙(OCP)에 위반됨
...
아래의 코드 예시처럼 중첩된 구조일 경우 `flatMap()` 메서드를 사용하고 단일 구조일 경우 `map()` 메서드를 사용한다.

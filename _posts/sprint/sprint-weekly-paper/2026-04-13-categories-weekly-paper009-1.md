...
`containerd` : 서버 환경 등에서 Docker 대신 컨테이너를 실행하고 관리하기 위해 사용할 수 있는 런타임 도구
...
단일 컨테이너는 `docker run` 명령어로 실행할 수 있지만, 실제 서비스는 웹 서버, API, 데이터베이스 등 다수의 컨테이너가 복합적으로 얽혀 동작한다.

수동으로 컨테이너들의 실행 순서를 맞추고 네트워크를 연결하는 것은 복잡하고 오류가 발생하기 쉽다.

컨테이너 오케스트레이션(ex: Docker Compose, Kubernetes)은 이러한 멀티 컨테이너의 토폴로지 정의, 실행, 연결 등 전반적인 생명주기를 통합 관리하기 위해 반드시 필요하다.

...
- 게시글 목록 조회 대상 Entity = `Post`
- `Post`가 참조하는 연관 Entity = `Author`
...
하나의 트랜잭션에서 **같은 조건의 조회 쿼리를 두 번 실행했을 때, 다른 트랜잭션의 변경과 커밋으로 처음에는 조회되지 않았던 데이터가 나타나거나, 조회되었던 데이터가 더 이상 조회되지 않는 현상**입니다.

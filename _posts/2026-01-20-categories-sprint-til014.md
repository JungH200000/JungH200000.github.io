...
5. `JCF*Service` 구현체와 `File*Service` 구현체의 공통점과 차이점
   - 공통점
     - 두 구현체 모두 검증/비지니스 로직/data Map에 데이터 저장
   - 차이점
     - `JCF*Serivce`는 data Map에 데이터를 저장하는 반면, `File*Serivce`는 한 단계 더 나아가 data Map에 데이터를 저장 후 파일에도 데이터 저장
